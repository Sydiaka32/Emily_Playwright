...
def test_publish_draft(navigate_to_my_auctions, allure_step):
    ...
    allure_step("Publish draft", my_auctions_page.publish_option(), take_screenshot=True)

    # Handle updating time
    page.wait_for_timeout(1500)

    # Refresh the current page
    allure_step("Refresh current page", page.reload(), take_screenshot=False)

    # Handle popup and updating time
    page.wait_for_timeout(3000)
    navigation_page.close_notification_if_exists()

    card_status = allure_step("Get auction status", my_auctions_page.get_card_published_status(), take_screenshot=True)
